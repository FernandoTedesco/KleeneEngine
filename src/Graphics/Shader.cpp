...
void Shader::CheckCompileErrors(unsigned int ID, const std::string& description,
				const std::string& type)
{
	...
	if (!success)
	{
	    ...
	    std::cout << "Attempted to compile the shader:" << description << " but failed"
		      << std::endl;
	...
	if (!success)
	{
	    ...
	    std::cout << "System Log:" << infoLog << std::endl;
	    std::cout << "Attempted to link shaders into a program object:" << description
		      << " but failed" << std::endl;
...
GLint Shader::GetUniformLocation(const std::string& key)
{
	...
	if (location == -1)
	{
	    ...
	    std::cout << "Could not find Uniform '" << key
		      << "' in the shader program:" << this->programID << std::endl;
	}
